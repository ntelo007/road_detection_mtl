...
def SpatialContextBlock(
    ...
    pooling_name = 'psp_level{}_pooling_task_{}'.format(level, task)
    conv_block_name = 'psp_level{}_task_{}'.format(level, task)
    upsampling_name = 'psp_level{}_upsampling_task_{}'.format(level, task)
...
def build_psp(
    ...
    for i in tasks:
        ...
        x1 = SpatialContextBlock(1, conv_filters, pooling_type, use_batchnorm, i)(x)
        x2 = SpatialContextBlock(2, conv_filters, pooling_type, use_batchnorm, i)(x)
        x3 = SpatialContextBlock(3, conv_filters, pooling_type, use_batchnorm, i)(x)
        x6 = SpatialContextBlock(6, conv_filters, pooling_type, use_batchnorm, i)(x)

        # aggregate spatial pyramid
        concat_axis = 3 if backend.image_data_format() == 'channels_last_{}'.format(i) else 1
        x = layers.Concatenate(axis=concat_axis, name='psp_concat_{}'.format(i))([x, x1, x2, x3, x6])
        x = Conv1x1BnReLU(conv_filters, use_batchnorm, name='aggregation_task_{}'.format(i))(x)

        # model regularization
        if dropout is not None:
            x = layers.SpatialDropout2D(dropout, name='spatial_dropout_task_{}'.format(i))(x)
        ...
        x = layers.Conv2D(
            ...
            name='final_conv_task_{}'.format(i),
        )(x)

        x = layers.UpSampling2D(final_upsampling_factor, name='final_upsampling_task_{}'.format(i), interpolation='bilinear')(x)
        x = layers.Activation(activation, name='{}'.format(activation+i))(x)

    model = models.Model(input_,
                        outputs= [branches[0], branches[1], branches[2]],
                        name='MTL_PSPnet')
